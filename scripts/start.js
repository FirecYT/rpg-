...
let mouseup = (e)=>{ // Кнопка мыши была поднята (вроде бы и ПКМ и ЛКМ)
	...
	if( pointInObj({x: e.layerX, y: e.layerY}, tmpRect(classes[4])) ) classes[4] = secrets.loli; // Лоли 2.0
	if( pointInObj({x: e.layerX, y: e.layerY}, tmpRect(classes[5])) ) classes[5] = secrets.dev; // Лоли 2.0

	if( pointInObj({x: e.layerX, y: e.layerY}, tmpRect(enter)) ) { // Если нажали на "Далее"
		...
		if (_class<=-1) { // Если класс не выбран (хз как будет что то меньше -1, но вдруг)
			...
			player = _classes_pref[_class]; // Установка класса
